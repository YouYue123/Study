#include <bits/stdc++.h>
using namespace std;
using ll = long long;
using ull = unsigned long long;

int solve(vector<vector<int>>& buttons, vector<int>& joggles) {
    int n = joggles.size();
    if (n == 0) return 0;

    queue<pair<vector<int>, int>> q;
    map<vector<int>, int> visited;
    
    vector<int> start(n, 0);
    q.push({start, 0});
    visited[start] = 0;

    while (!q.empty()) {
        auto [state, presses] = q.front();
        cout << "presses: " << presses << endl;
        q.pop();
        bool reached = true;
        for (int i = 0; i < n; i++) {
            if (state[i] != joggles[i]) {
                reached = false;
                break;
            }
        }
        if (reached)  return presses;
        for (auto& button : buttons) {
            vector<int> new_state = state;
            for (int counter : button) {
                if (counter < n) {
                    new_state[counter]++;
                }
            }
            bool valid = true;
            for (int i = 0; i < n; i++) {
                if (new_state[i] > joggles[i]) {
                    valid = false;
                    break;
                }
            }
            if (valid) {
                if (visited.find(new_state) == visited.end() || visited[new_state] > presses + 1) {
                    visited[new_state] = presses + 1;
                    q.push({new_state, presses + 1});
                }
            }
        }
    }
    return -1; 
}

int main() {
    auto start = chrono::high_resolution_clock::now();
    ull ans = 0;
    
    ios::sync_with_stdio(false);
    cin.tie(nullptr);
    string line;
    while(getline(cin, line)) {
        if(line.empty()) continue;
        
        string target;
        vector<vector<int>> buttons;
        vector<int> joggles;
        // Parse target
        int start_pos = line.find('[');
        int end_pos = line.find(']');
        if(start_pos != string::npos && end_pos != string::npos) {
            target = line.substr(start_pos + 1, end_pos - start_pos - 1);
        } else {
            continue;
        }
        // Parse buttons
        int pos = end_pos + 1;
        while(pos < line.length()) {
            pos = line.find('(', pos);
            if(pos == string::npos || line[pos] == '{') break;
            int button_end = line.find(')', pos);
            if(button_end == string::npos) break;
            string button_str = line.substr(pos + 1, button_end - pos - 1);
            vector<int> button;
            stringstream button_ss(button_str);
            string num_str;
            while(getline(button_ss, num_str, ',')) {
                num_str.erase(remove_if(num_str.begin(), num_str.end(), ::isspace), num_str.end());
                if (!num_str.empty()) {
                    button.push_back(stoi(num_str));
                }
            }
            buttons.push_back(button);
            pos = button_end + 1;
        }
        // Parse joggles
        pos = end_pos + 1;
        while(pos < line.length()) {
            pos = line.find('{', pos);
            int joggle_end = line.find('}', pos);
            if(joggle_end == string::npos) break;
            string joggle_str = line.substr(pos + 1, joggle_end - pos - 1);
            stringstream joggle_ss(joggle_str);
            string num_str;
            while(getline(joggle_ss, num_str, ',')) {
                num_str.erase(remove_if(num_str.begin(), num_str.end(), ::isspace), num_str.end());
                if (!num_str.empty()) {
                    joggles.push_back(stoi(num_str));
                }
            }
            pos = joggle_end + 1;
        }
        ans += solve(buttons, joggles);
    }
    
    cout << ans << endl;
    auto end = chrono::high_resolution_clock::now();
    auto duration = chrono::duration_cast<chrono::nanoseconds>(end - start);
    cout << "runtime: " << duration.count() << " nanoseconds" << endl;
}